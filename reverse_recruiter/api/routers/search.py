...
from reverse_recruiter.services.search_service import SearchServiceError

router = APIRouter(prefix="/search", tags=["search"])


def _to_row(job) -> JobRow:
...
async def run_search(body: SearchRequest) -> list[JobRow]:
    ...
    try:
        jobs = await svc.run_search(body.model_dump(exclude={"use_llm"}), use_llm=body.use_llm)
        ...
        raise HTTPException(status_code=status, detail={"error": e.code, "message": e.message}) from e


@router.get("/saved")
async def list_saved() -> list[dict]:
    svc = get_saved_search_service()
    items = await svc.list()
    return [s.model_dump(mode="json") for s in items]


@router.post("/saved")
async def save_search(body: SavedSearchCreateRequest) -> dict:
    svc = get_saved_search_service()
...
async def get_settings() -> SettingsResponse:
    data = await get_settings_store().get()
