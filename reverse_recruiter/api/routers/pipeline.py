...
async def list_pipeline(
    lifecycle: str | None = None, include_rejected: bool = False
) -> list[dict]:
...
@router.post("/confirm")
async def confirm(body: ConfirmRequest) -> list[dict]:
    svc = get_pipeline_service()
    try:
        jobs = await svc.confirm(body.job_ids, body.action)
        return [j.model_dump(mode="json") for j in jobs]
    except PipelineServiceError as e:
        raise HTTPException(status_code=400, detail={"error": e.code, "message": e.message}) from e
...
@router.patch("/{job_id}")
async def update_job(job_id: str, body: PipelineUpdateRequest) -> dict:
    svc = get_pipeline_service()
...
async def add_interview(job_id: str, body: InterviewCreateRequest) -> dict:
    svc = get_pipeline_service()
    try:
...
async def update_interview(
    job_id: str, event_id: str, body: InterviewUpdateRequest
) -> dict:
    svc = get_pipeline_service()
...
async def delete_interview(job_id: str, event_id: str) -> dict:
    svc = get_pipeline_service()
    try:
